...
// Track active index with promise
// const trackIndex = async (index: number) => {
//   await new Promise(() => console.log(`Active Index Updated: ${index}`));
// };
...
function App() {
  ...
  //// ** Help track the current active index with useEffect ** ////
  // useEffect(() => {
  ...
  //// ** Track active index with Promise  ** ////
  // const handleTrackActiveIndex = async () => {
  //   await trackIndex(activeIndex);
  // };

  // handleTrackActiveIndex();

  const shuffleCards = () => {
    //  Shuffle all cards except the first one
    const shuffled = originalCardData.slice(1).sort(() => Math.random() - 0.5);
    // Return the shuffled cards, keeping the first card on top
    return [originalCardData[0], ...shuffled];
  };
  ...
  const increaseScore = () => {
    if (scoreCount < activeIndex && !scoreUpdated && isFlipped) {
  ...
  return (
    <div className='w-screen h-screen p-10'>
      ...
      <div className='flex justify-center p-8'>
        <div className='w-auto h-auto grid grid-cols-3 gap-11'>
          ...
          {/* DECK */}
          <div className='w-48 h-72 text-white bg-[#171717] grid justify-center content-center rounded-lg rotate-6'>
            <Deck
      ...
      <div className='w-auto h-auto grid grid-cols-5 gap-11'>
        <div className='grid'></div>
        <div className='grid'></div>
        {/* REVEAL BUTTON */}
        <Button
          className='grid border-solid border-2 rounded-lg transition hover:bg-[#283348] active:translate-y-1 ease-out duration-300'
          onClick={() => {
      ...
      {gameOver && isFlipped && (
        <h3 className='pl-80 text-green-300'>{nextGame}</h3>
      )}
    </div>
  );
}
